...
import { getQueue, getCurrentBatch, addQueue, mintNft } from '@/services/Web3Services';
...
useEffect(() => { 
  ...
  return (
    ...
    <p className="mt-[40px] mb-[40px] leading-tight font-Agency text-[50px] sm:text-[80px] font-normal w-full">Bitcoin AiD Protocol - NFT Payment Queue</p>
    <div className='mx-auto w-[35%] bg-[#26251f35] rounded-3xl mb-[10px] flex flex-col p-[10px] '>
        ...
        <Image src="/images/NFTSATOSHI.png" alt="NFT" width={1000} height={1000} className='mx-auto max-w-[60%] max-h-[55%]'></Image>
        <p className='font-Agency mx-auto text-[25px] mt-[10px]'>$10</p>
        <button onClick={buyNft} className='mx-auto p-[10px] w-[200px] bg-[#d79920] rounded-full mt-[10px]'>Buy Nft</button>
    </div>
...
<div className='flex flex-col md:flex-row md:justify-between md:gap-6 mb-[30px]'>
  ...
  <p className='font-Agency text-center text-[25px] mt-[20px]'>Add your NFT's to Queue</p>
  <div className='w-full flex justify-center'>
    <button onClick={openAddNft} className='bg-[#d79920] w-[200px] p-[10px] rounded-full mb-[20px]'>
...
<div className='mx-auto md:w-[40%] w-full bg-[#26251f35] h-[200px] mt-[20px] flex flex-col justify-between items-center p-4'>
  <p className='font-Agency text-center text-[25px] mt-[20px]'>Claim NFT's Rewards</p>
  <div className='w-full flex justify-center'>
...
<div className="mx-auto max-w-[100%] overflow-y-auto custom-scroll max-h-[300px] slider-container p-2 ml-[30px] mb-[100px] mt-[100px]">
    {queueData.map((dataSet, index) => (
      <div key={index} className="mb-4">
        ...
        <Slider {...settings(dataSet.length)} 
          ...
          {dataSet.map((item, itemIndex) => (
            <div key={itemIndex} className="mr-[10px]">
              <div className="bg-[#d79920] p-4 transform transition-transform duration-300 h-[200px] sm:w-[260px] w-[230px] hover:scale-105 hover:rotate-1 hover:shadow-lg hover:bg-[#d79a20f2]">
                <div className=''><h3>Posição da Fila: {itemIndex + 1}</h3></div>
                <p>User: <span> {item.user ? `${item.user.slice(0, 6)}...${item.user.slice(-4)}` : 'N/A'}</span></p>
...
{addNftOpen ? (
           ...
           <div className="fixed inset-0 bg-black opacity-80" onClick={openAddNft}></div>
           <div className="relative bg-[#201f1b] border-2 border-[#eda921] p-6 rounded-lg shadow-lg w-[80%] max-w-lg z-10">
             ...
             <p className="text-center text-white text-[30px] font-Agency">ADD NFT QUEUE</p>
                <input className="w-full m-w-[90%] p-2 bg-[#33322d] rounded-3xl mt-[20px] focus:outline-none focus:border-2 focus:border-[#eda921]" value={inputValue} onChange={handleInputChange} type="number" placeholder="Which Batch"></input>
              <div className="w-full flex flex-col items-center mb-[15px] mt-[20px]">
                <button onClick={handleSubmit} className="w-[150px] font-semibold rounded-3xl bg-[#eda921] p-[8px]">Go Queue</button>
              </div>
           </div>
